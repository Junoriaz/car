<transition name='fade'>
    ...
    <div class="fix" :class="{fixIn:navBool}" @click="hideFix" id="Js-fix">
        ...
        <ul id="yearUl" v-show="cartypeArr[1].fixBool">
            <li v-for="date in dateArr" @click='conditionFun($event,cartypeArr,1,"","",date.date)'><a href="javascript:void(0)">{{date.date}}</a></li>
        </ul>
        <!---外色--->
        <ul id="outColorUl" v-show="cartypeArr[2].fixBool">
            <li v-for="outColor in outColorArr" @click='conditionFun($event,cartypeArr,2,"","",outColor.color)'><a href="javascript:void(0)">{{outColor.color}}</a></li>
        </ul>

        <!---内色--->
        <ul id="inColorUl" v-show="cartypeArr[3].fixBool">
            <li v-for="inColor in inColorArr" @click='conditionFun($event,cartypeArr,3,"","",inColor.color)'><a href="javascript:void(0)">{{inColor.color}}</a></li>
        </ul>
        <!---卖到哪里-->
        <ul id="whereUl" v-show="cartype2Arr[0].fixBool">
             <li v-for="area in areaArr" @click='conditionFun($event,cartype2Arr,0,"","",area.area)'><a href="javascript:void(0)">{{area.area}}</a></li>
        </ul>
        <!---状态-->
        <ul id="statusUl" v-show="cartype2Arr[1].fixBool">
            <li v-for="payment in paymentArr" @click='conditionFun($event,cartype2Arr,1,"","",payment.payment)'><a href="javascript:void(0)">{{payment.payment}}</a></li>
        </ul>
    </div>
